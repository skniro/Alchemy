...
public class ChairBlock extends HorizontalFacingBlock {
    ...
    public ActionResult onUse(BlockState state, World level, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hitResult) {
        if(!level.isClient()) {
            Entity entity = null;
            List<ChairEntity> entities = level.getEntitiesByType(AlchemyEntityType.CHAIR_ENTITY, new Box(pos), chair -> true);
            if(entities.isEmpty()) {
                entity = AlchemyEntityType.CHAIR_ENTITY.spawn(((ServerWorld) level), pos, SpawnReason.TRIGGERED);
            } else {
